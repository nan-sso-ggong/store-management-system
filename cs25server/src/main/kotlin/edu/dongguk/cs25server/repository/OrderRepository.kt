...
interface OrderRepository: JpaRepository<Order, Long> {
    @Query(value = "SELECT DISTINCT o.customer.name AS CNAME, o.id AS OID, " +
            ...
            "WHERE o.store = :store AND (:state IS NULL OR o.orderStatus = :state) AND (:startDate IS NULL OR o.createdAt >= :startDate) AND (:endDate IS NULL OR o.createdAt <= :endDate)",
